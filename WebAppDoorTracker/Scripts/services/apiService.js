DoorTrackerApp.factory('apiService', apiService);
...
function apiService($http, $location, notificationService, $rootScope) {
    ...
    function post(url, data, success, failure) {
        ...
        return $http.post(url, data).then(function (result) {
            ...
            if (error.status == '401') {
                notificationService.displayError('Authentication required.');
                $rootScope.previousState = $location.path(); $location.path('/login');
            } else if (failure != null) {
                failure(error);
            }
        });
    ...
    function patch(url, data, success, failure) {
        ...
        return $http.patch(url, data).then(function (result) {
            success(result);
        }, function (error) {
            if (error.status == '405') {
                notificationService.displayError('Method not allowed.');
                $rootScope.previousState = $location.path(); $location.path('/login');
            } else if (failure != null) {
                failure(error);
            }
